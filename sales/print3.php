...
<div class="col-md-3 col-lg-3 round">
    ...
    $asd = $row['taxno'];
    $logosd = $row['en']; ?>
    <table class="table" style="width:38%; font-size:16px;">

    <tr>
    <td colspan="5" style="font-size: 15px;" align="center"><?php echo $logo; ?></td>
    </tr>

    
    <tr>
    <td colspan="5" style="font-size: 15px;" align="center"><?php echo $logosd; ?></td>
    </tr>
       <tr>
            <Td  align="center"  style="font-size: 18px;" colspan="4">رقم الفاتورة <?php echo $_GET['print']; ?> </Td>
        </tr>
    ...
    <br>
    <?php include("dbcon/dbcon.php");
    $sql = mysql_query("select * ,price,sum(qty),type  ,name from sales where invoice='$_GET[print]' and tell ='0' Group by name");
    $row = mysql_fetch_array($sql); ?>
    <tr>
    <Td align="center" style="font-size: 18px;" colspan="4">فاتورة مرتجعات <?php 
    if($row['type']==1)
    {
        echo "نقدا";
    }
    ...
    elseif($row['type']==3)
    {
        ...
        <tr>
            ...
            <td colspan="2">
            <?php include("dbcon/dbcon.php");
                                $sql = mysql_query("select count(qty) from sales where invoice='$_GET[print]'  and tell ='0'");
                                $row = mysql_fetch_array($sql);
        ...
        <td colspan="3"><strong>الإجمالي</strong></td>
            <td colspan="2"> <?php include("dbcon/dbcon.php");
                                $sql = mysql_query("select sum(qty*price) from sales where invoice='$_GET[print]' and tell ='0' ");
                                $row = mysql_fetch_array($sql);
                                 $sum1 =round($row['sum(qty*price)']-$row['sum(qty*price)']*15/100,2); 
