...
if (isset($_POST['operation']) && $_POST['operation'] == "add_product") {
    ...
    VALUES ('" . date("Y-m-d H:i:s") . "','" . date("Y-m-d H:i:s") . "','" . $_POST['invoice_id'] . "', '" . $products['barco'] . "' , '1'  , '" . $products['name'] . "', '" . $products['price'] . "', '0'  ,  '" . $products['unit'] . "' ,  '" . $_POST['payment'] . "'  ,    '0'  ,  '" . $vat['tax'] . "'    , '1' )";
    ...
    $sql = mysqli_query($connect, "select *from sales where invoice='" . $_POST["invoice_id"] . "'  and tell !=0");
    // $row = mysqli_fetch_array($sql);
    $total = 0;
    $vat_result = 0;
    $sub_total = 0;
    ...
    while ($row = mysqli_fetch_array($sql)) {
        $vat_result += round(($row['price'] * $row['qty']*$tax/100 ), 2);
        $total += round(($row['price'] * $row['qty']), 2);
        $sub_total +=  round(($row['price'] * $row['qty']), 2);
        $table .= '<tr align="center">
                        <td style="width:25%;">' . $row['name'] . '</td>
                        <td style="width:15%;">' . $row['category'] . '</td>
                        <td style="width:10%;"><i style="color:green;" class=" fas fa-plus" onclick=javascript:add_product(' . $row["product"] . ');></i> ' . $row['qty'] . ' <i  style="color:red;" class=" fas fa-minus" onclick=javascript:decrease(' . $row["product"] . ');></i></td>
                        <td style="width:10%;">' . $row['price'] . '</td>
                        <td style="width:10%;">' . round(($row['price'] * $row['qty']), 2) . '</td>
                        <td style="width:10%;">' . round(($row['price'] * $row['qty'] * $tax / 100), 2) . '</td>
                        <td style="width:10%;">' . round(($row['price'] * $row['qty']  ), 2) . '</td>
...
if (isset($_POST['operation']) && $_POST['operation'] == "add_unknown_product") {
    ...
    $sql = "INSERT INTO `sales` (`datetime`,    `invoice`,   `product`   , `qty`  , `name`   , `price`,  `discount` ,   `category`,  `type`, `user`,   `vat`,       `tell`)
    VALUES ('" . date("Y-m-d H:i:s") . "','" . $_POST['invoice_id'] . "', '" . $barco . "' , '" . $_POST['qyt'] . "' , '" . $name . "', '" . $price . "', '0'  ,  'وحده' ,    '" . $_POST['payment'] . "'  ,    '0'  ,  '" . $vat['tax'] . "'    , '1' )";
    ...
    $sql = mysqli_query($connect, "select *from sales where invoice='" . $_POST["invoice_id"] . "'  and tell ='1' ");
    // $row = mysqli_fetch_array($sql);
    $total = 0;
    $vat_result = 0;
    $sub_total = 0;
...
if (isset($_POST['operation']) && $_POST['operation'] == "decrease_product_qyt") {
    ...
    $delete_sql = "UPDATE sales set tell ='0' WHERE   transaction_id = '" . $transaction_id . "'";
    ...
    $sql = mysqli_query($connect, "select *,qty,invoice,name,category,product,price,discount,transaction_id from sales
     where invoice='" . $_POST["invoice_id"] . "' and tell ='1' ");
    // $row = mysqli_fetch_array($sql);
    $total = 0;
    $vat_result = 0;
    $sub_total = 0;
...
if (isset($_POST['operation']) && $_POST['operation'] == "delete_invoice") {
    ...
    $sql_sales = mysqli_query($connect, "SELECT qty as qyt , product FROM sales WHERE invoice = '2020000174'  and tell ='1'  product");
    ...
    //  // delete item form invoice  
    $delete_sql = "UPDATE sales set tell ='0' WHERE   invoice = '" . $invoice_id . "'";
    mysqli_query($connect, $delete_sql);
    ...
    $sql = mysqli_query($connect, "select qty,invoice,name,category,product,price,discount,transaction_id from sales where invoice='" . $_POST["invoice_id"] . "' and tell !=0 ");
    // $row = mysqli_fetch_array($sql);
    $total = 0;
    $vat_result = 0;
    $sub_total = 0;
...
if (isset($_POST['operation']) && $_POST['operation'] == "delete_product") {
    ...
    $sql = mysqli_query($connect, "SELECT qty as total FROM sales WHERE  invoice = '" . $invoice_id . "' AND product = '" . $barcode . "' and tell ='1'");
    $result = mysqli_fetch_array($sql);
    ...
    // delete item form invoice  
    $delete_sql = "UPDATE sales set tell ='0'  WHERE   invoice = '" . $invoice_id . "' AND product= '" . $barcode . "'";
    mysqli_query($connect, $delete_sql);

    $sql = mysqli_query($connect, "select sum(price*qty) from sales where invoice='" . $_POST['invoice_id'] . "' and tell !=0");
    $row = mysqli_fetch_array($sql);
...
if (isset($_POST['operation']) && $_POST['operation'] == "loading") {
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $table .= '<tr align="Center">
                        <td>' . $row['name'] . '</td>
                        <td>' . $row['category'] . '</td>
                        <td><i style="color:green;" class=" fas fa-plus" onclick=javascript:add_product(' . $row["product"] . ');></i> ' . $row['qty'] . ' <i  style="color:red;" class=" fas fa-minus" onclick=javascript:decrease(' . $row["product"] . ');></i></td>
                       <td>' . $row['price'] . '</td>
                        <td>' . round(($row['price'] * $row['qty']), 2) . '</td>
                        <td>' . round(($row['price'] * $row['qty'] * $tax / 100), 2) . '</td>
                        <td>' . round(($row['price'] * $row['qty']), 2) . '</td>
