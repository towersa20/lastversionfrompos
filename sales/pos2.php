<!-- VALUES ('" . $_SESSION['SESS_MEMBER_ID'] . "', '" . $products['pid'] . "' , '1'  , '" . $products['product_name'] . "', '" . $products['product_price'] . "', '0'  ,  '" . $products['unit'] . "' ,    '" . $_POST['payment'] . "'  ,    '0'  ,  '" . $vat['tax'] . "'    , '" . $products['tell'] . "' , '1')"; -->
<!--  -->
<?php
...
if (isset($_GET['SESS_MEMBER_ID'])) {
    ...
    $_SESSION['SESS_MEMBER_ID'] = $_GET['SESS_MEMBER_ID'];
}


include('dbcon/dbcon.php');
if (isset($_SESSION['SESS_MEMBER_ID']))
    $search = $_SESSION['SESS_MEMBER_ID'];
...
if (isset($_POST['info'])) {
    ...
    // get the products counts form store based on product name
    // $products_results = mysql_query("select * from products WHERE  pid = '" . $_POST['info'] . "' LIMIT 1 ");
    $products_results = mysql_query("select * from storing WHERE  barco = '" . $_POST['info'] . "' LIMIT 1 ");

    $vat_resulte = mysql_query("SELECT * FROM vat WHERE  vid = 1 LIMIT 1 ");
    $vat = mysql_fetch_array($vat_resulte);

    $products = mysql_fetch_array($products_results);


    if ($products['sid'] > 0) {
        $sql = "INSERT INTO `sales` (`invoice`,           `product`            , `qty`    , `name`                     , `price`,                                      `discount` ,      `category`,             `type`, `user`,   `vat`,       `tell`, `customer_id`)
        VALUES ('" . $_SESSION['SESS_MEMBER_ID'] . "', '" . $products['barco'] . "' , '1'  , '" . $products['name'] . "', '" . $products['price'] . "', '0'  ,  '" . $products['unit'] . "' ,    '" . $_POST['payment'] . "'  ,    '0'  ,  '" . $vat['tax'] . "'    , '" . $products['tell'] . "' , '1')";

        $result = mysql_query($sql);

        $qyt =  $products['qunt'] - 1;
        $update_prodect_qyt_sql = "UPDATE `storing` SET `qunt` = '".$qyt."' WHERE `storing`.`sid` = ".$products['sid'];
        mysql_query($update_prodect_qyt_sql);
        mysql_error();
        // die;



        $_SESSION['message'] = "تمت اضافة المنتج";

        echo "<h2  style='color:green ; text-align: center;'>" . $_SESSION['message'] . "</h2>";
    } else {


        $_SESSION['message'] = "<h2  style='color:red ; text-align: center;'>عفوا المنتج غير متوفر الرجاء ادخال البيانات بشكل صحيح</h2>";
        echo $_SESSION['message'];
    }

    header("location: posss.php?SESS_MEMBER_ID=" . $_GET['SESS_MEMBER_ID']);
}

if (isset($_GET['del'])) {

    $sql = mysql_query("select sum(qty) as total from sales where product=" . $_GET['del']);
    $result = mysql_fetch_array($sql);
    print_r($result);
    die;
    $sql = "DELETE FROM `sales` WHERE `product` = " . $_GET['del'];
    $result = mysql_query($sql);

    header("location:posss.php?SESS_MEMBER_ID=" . $_SESSION['SESS_MEMBER_ID']);
}
?>
...
// $new  = mysql_query("select * from job");
// $show = mysql_fetch_array($new);
// $search = $show['name'];
?>
...
<div class="container-fluid">
    <div class="row">
        ...
        $sql = mysql_query("select sum(price*qty) from sales where invoice='$search'");
    ...
    <div class="row">
        <div class="col-md-10 col-lg-10 ">
            <div class="card-box">
                ...
                <form action="posss.php?SESS_MEMBER_ID=<?= $_SESSION['SESS_MEMBER_ID'] ?>" method="POST">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <td style="width:10%;">
                                <div class="input-group">
                                    <button class="btn btn" name="add_search" style="width: 100%; background-color: #1FAF2E;  font-family: 'Droid Arabic Kufi'; ">
                                        بحث <i class="fa fa-search"></i></button>
                            </td>
                            <td style="width: 12%;">رقم الفاتورة</td>
                            <td style="width: 20%;">
                                <input type="text" class="form-control" style="font-size: 15px; color:red;" value="<?php echo $_SESSION['SESS_MEMBER_ID']; ?>" name="b" class=" btn-rounded">
                            </td>
                    ...
                    <input type="hidden" class="form-control" style="font-size: 10px;" value="<?php echo $_SESSION['SESS_MEMBER_ID']; ?>" name="b" class=" btn-rounded">
                    <input type="hidden" name="c" class="form-control" style="font-size: 10px;" value="<?php echo date('Y-m-d'); ?>" class=" btn-rounded"><!-- نهاية حقل التاريخ -->
                    ...
                    <table class="table table-bordered tab-content table-embed-responsive" style="background-color: #eff5fc; width: 100%; font-family: 'Droid Arabic Kufi'; font-size:12px; ">
                        ...
                        include("dbcon/dbcon.php");
                        //عرض بيانات الفاتورة المخزن بشاشة المبيعات
                        $sql = mysql_query("select sum(qty),invoice,name,category,product,price,discount,transaction_id from sales where invoice='" . $_SESSION["SESS_MEMBER_ID"] . "' GROUP BY NAME");
                        $row = mysql_fetch_array($sql);
                        //نهاية الكود
                        ?>
                        <?php
                        ...
                        do { ?>
                            <tr>
                                ...
                                <td align="center" style="width:18%;">
                                    ...
                                    <div class="container-fluid">
                                        <div class="row">
                                            <div class="col-3"><a href="me1.php?max=<?php echo $pro = $row['product']; ?>&max2=1&transaction_id=<?php echo $row['transaction_id']; ?>">
                                                    <img src="1.png" style="width:20px;"></a></div>

                                            <div class="col-6" align="center">
                                                <input type="text" value="<?php echo $row['sum(qty)']; ?>" class="form-control" style="font-size:20px;width:62px; color:red; height:33px;">
                                            </div>
                                            <div class="col-3">
                                                <a href="me3.php?min=<?php echo $row['product']; ?>&max2=1&transaction_id=<?php echo $row['transaction_id']; ?>">
                                ...
                                <td style="font-size:14px;" align="center"><?php $endsum = $row['price'] * $row['sum(qty)'] * $tax / 100 + $row['price'] * $row['sum(qty)'];
                                                                            echo round($endsum, 2);  // 1.96
                                                                            ?></td>
                                <!-- FIX: -->
                                <td align="center"><a href="posss.php?del=<?php echo $row['product']; ?>&invoice=<?= $_GET[''] ?>"><i style="color:red;font-size:20px;" class="fas fa-times-circle"></i></a>
                                </td>
                            </tr>
        ...
        <div class="col-md-2 col-lg-2">
            <div class="card-box">
                <table class="table table-bordered" style="background-color: #eff5fc; font-family: 'Droid Arabic Kufi'; font-size:14px; ">
                    <tr>
                        <td style="width:16.6%;">
                            <a href="print2.php?print=<?php echo  $_SESSION['SESS_MEMBER_ID'];  ?>">
                    ...
                    <tr>
                        <td style="width:16.6%;">
                            <a href="poss2.php?invoice=<?php echo $_SESSION['SESS_MEMBER_ID']; ?>">
                    ...
                    <tr>
                        ...
                        <td style="width:16.6%;">
                            <a href="posss.php?delete=<?php echo $_SESSION['SESS_MEMBER_ID']; ?>&&maxi=<?php echo $pro; ?>" onclick="return confirm('هل تريد حذف الفاتورة');">
                    ...
                    <tr>
                        ...
                        <td style="width:16.6%;">
                            <a href="refund.php" target="_blank">
                                <button type="button" style="background-color:#CCC; font-size:10px; height:33px; width:100%;  font-family: 'Droid Arabic Kufi';" class="btn btn-succes">
                                    <i class=" far fa-stop-circle "></i> راجعه
                                </button>
                            </a></td>
                        ...
                        <tr>
                            <td colspan="3">
                                ...
                                <datalist id="searchrxt">
                                    ...
                                    <option></option>
                                    <?php do { ?>
                                        <option value="<?php echo $res['barco']; ?>"><?php echo $res['name']; ?> <i class="fa fa-shopping-bag"></i> <?php echo $res['barco']; ?></option>
            ...
            <div align="center" style="font-size:16px; border-radius:0.7em;  color:#707070;" class="card-box">
                ...
                <table class="table table-borderd" style="background-color: #eff5fc; font-family: 'Droid Arabic Kufi'; font-size:14px; ">
                    ...
                    <tr name="addem" action="" id="addem">
                        <td colspan="4" align="center">
                            <select name="payment" class="form-control">
                                <option <?php if ($_SESSION['payment']  == 1) :  ?> selected <?php endif; ?> value="1">نقدا</option>
                                <option <?php if ($_SESSION['payment']  == 2) :  ?> selected <?php endif; ?> value="2">فيزا</option>
                                <option <?php if ($_SESSION['payment']  == 3) :  ?> selected <?php endif; ?> value="3">اجل</option>
                    ...
                    <tr align="center">
                        ...
                        <td colspan="2">
                            <button type="button" style="background-color:#DD5656; font-size:12px;  width:100%;  font-family: 'Droid Arabic Kufi';" class="btn  btn"></i> المتبقي
                            </button>
                        </td>
                    </tr>
                    ...
                    <tr align="center">
                        ...
                        <input name="prices" id="prices" type="hidden" value="<?php echo round($costs, 2);  // 1.96 
                                                                                ?>" />
                        ...
                        <input type="hidden" id="tb2" value="<?php echo round($costs, 2); ?>" name="tb2" onkeyup="calc(this)" />
...
$sql2=mysql_query("select count(qty) from sales where invoice='$_POST[search]' and name='غير معروف'");
$rowk=mysql_fetch_array($sql2);
echo $rowk['count(qty)'];?></td>
<td>الإجمالي</td>
<td><?php include('dbcon/dbcon.php');
$sql2=mysql_query("select sum(price*qty) from sales where invoice='$_POST[search]' and name='غير معروف'");
$rowk=mysql_fetch_array($sql2);
...
invoice='$_POST[search]' and name='غير معروف'");
...
do { ?>
             ...
             <td align="center"><a href="posss.php?del=<?php echo $rows['transaction_id'] ;?>"><i style="color:red;font-size:20px;" class="fas fa-times-circle"></i></a></td>
...
<?php } while($rows=mysql_fetch_array($sql1))?>
        ...
        <div class="col-md-2 col-lg-2 round">
            <div align="center" class="card-box" style="border-radius:2.7em;">
                <button type="button" class="btn btn-round" style="border-color:#707070; color:#707070; width:100%; font-family: 'Droid Arabic Kufi'; font-size:19px; border-radius:0.7em;">
                    الإجمالي
                    <hr>
                    <strong style="color:red;"><?php echo round($costs, 2);  // 1.96   
                                                ?></p></button>
            </div>
            <br><br><br>
                ...
                <table class="table table-bordered tab-content table-embed-responsive" style="background-color: #eff5fc; width: 100%; font-family: 'Droid Arabic Kufi'; font-size:15px; ">
                    ...
                    <tr>
                        <td><a href="../System/index.php">
                                <button type="button" style="background-color:#707070; color:#eff5fc; font-size:15px; height:28px;  width:100%;  font-family: 'Droid Arabic Kufi';" class="btn btn-succes btn">
    ...
    <div class="col-md-12 col-lg-12 round">
        <div align="center" class="card-box" style="border-radius:1.7em;">
            <table class="table table-bordered">
                <tr>
                    <td><a href="posss.php?unknown=1&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 1
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=2&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 2
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=3&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 3
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=4&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 4
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=5&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 5
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=6&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 6
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=7&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 7
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=8&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 8
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=9&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 9
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=10&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 10
                            </button>
                        </a></td>
                </tr>
                <tr>
                    <td><a href="posss.php?unknown=10&ks1=غير معروف">
                            <button type="button" type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 10
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=20&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 20
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=30&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 30
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=40&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 40
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=50&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 50
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=60&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 60
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=70&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 70
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=80&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 80
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=90&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 90
                            </button>
                        </a></td>
                    <td><a href="posss.php?unknown=100&ks1=غير معروف">
                            <button type="button" class="btn btn-primary btn-rounded" style="width:90px; font-size:16px;"><i class=" fab fa-accusoft "></i> 100
                            </button>
                        </a></td>
                </tr>
            </table>
    ...
    <div class="modal-dialog modal-side modal-top-right" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="table table-bordered">
                    <tr>
                        <td colspan="2">قائمة المبيعات <?php echo $_SESSION['SESS_MEMBER_ID'] + 1; ?>
                    </tr>

                    <input type="hidden" class="form-control" style="font-size: 10px;" value="<?php echo $_SESSION['SESS_MEMBER_ID']; ?>" name="it5" class=" btn-rounded">
            ...
            <div class="modal-footer">
                <button type="button" style="background-color: #DD5656; width:120px; font-family: 'Droid Arabic Kufi';" class="btn btn-rounded " data-dismiss="modal"><i class=" fas fa-times "></i> الغاء
                </button>
                <button type="submit" name="okpos" style="background-color: #1FAF2E; width: 120px; font-family: 'Droid Arabic Kufi';" class="btn btn-rounded "><i class=" fab fa-accusoft "></i> حفظ
                </button>
            </div>
        </div>
    </div>
