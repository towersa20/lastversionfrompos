...
if (isset($_POST['operation']) && $_POST['operation'] == "add_product") {
    ...
    VALUES ('" . date("Y-m-d H:i:s") . "','" . $_POST['invoice_id'] . "', '" . $products['barco'] . "' , '1'  , '" . $products['name'] . "', '" . $products['price'] . "', '0'  ,  '" . $products['unit'] . "' ,    '" . $_POST['payment'] . "'  ,    '0'  ,  '" . $vat['tax'] . "'    , '" . $products['tell'] . "' )";
...
if (isset($_POST['operation']) && $_POST['operation'] == "add_unknown_product") {
    ...
    $sql = "INSERT INTO `sales` (`datetime`,    `invoice`,   `product`   , `qty`  , `name`   , `price`,  `discount` ,   `category`,  `type`, `user`,   `vat`,       `tell`)
    VALUES ('" . date("Y-m-d H:i:s") . "','" . $_POST['invoice_id'] . "', '" . $barco . "' , '" . $_POST['qyt'] . "' , '" . $name . "', '" . $price . "', '0'  ,  'وحده' ,    '" . $_POST['payment'] . "'  ,    '0'  ,  '0'    , '11' )";
...
if (isset($_POST['operation']) && $_POST['operation'] == "decrease_product_qyt") {
    ...
    $sql = mysqli_query($connect, "SELECT * FROM sales WHERE  invoice = '" . $invoice_id . "' AND product = '" . $barcode . "'");
...
if (isset($_POST['operation']) && $_POST['operation'] == "delete_invoice") {
    ...
    $sql_sales = mysqli_query($connect, "SELECT sum(qty) as qyt , product FROM sales WHERE invoice = '2020000174' GROUP BY product");
...
if (isset($_POST['operation']) && $_POST['operation'] == "delete_product") {
    ...
    $sql = mysqli_query($connect, "SELECT sum(qty) as total FROM sales WHERE  invoice = '" . $invoice_id . "' AND product = '" . $barcode . "'");
    $result = mysqli_fetch_array($sql);
