...
mysql_select_db("infos",$dbs);
				...
				<input type="text" name="pid[]" autofocus title="barcode" placeholder="barcode" style="border:0px; height:36px;" class="form-control big-shadow pid" autofocus required id="search" placeholder="باركود" list="searchrxt" autocomplete="off">
	<datalist id="searchrxt">
        <?php do {?>
		            	
		            		<option value="<?php echo $rows['pid']; ?>"><?php echo $rows["pid"]; ?></option><?php
		            	}while($rows=mysql_fetch_array($sql));
		            ?>
</datalist>
		    </td>
		    <td><input name="product_name[]" title="Product" type="text"  style="border:0px; " class="form-control big-shadow pro_name" required></td>
		    <td><input name="unit[]" type="text"  title="Unit type" style="border:0px; " class="form-control big-shadow unit" required></td>
		    <td><input name="qty[]" title="الكمية الكلية للصنف الواحد" type="number" min="1" style="border:0px; " class="form-control big-shadow qty" required></td>
		    <td><input name="price[]" step="0.1" readonly="off"  title="سعر الشراء قبل الضريبة للوحده الواحده" type="number"  min="1" style="border:0px; "  class="form-control big-shadow price" ></td>
		    <td><input name="pays[]" step="0.1" required  title="سعر البيع" type="number"  min="1" style="border:0px; "  class="form-control big-shadow" ></td>
			...
			<input name="date[]"  title="تاريخ التاريخ" type="hidden"  value="20<?php echo date('y-m-d');?>" style="border:0px; " class="form-control big-shadow" ></td>
	<span></tr>
<?php
	exit();
}
