...
Invoice.prototype = {
    ...
    calcGrandTotal: function () {
        var grandTotal = Number(jQuery($.opt.subtotal).html())
                       ...
                       grandTotal = self.roundNumber(grandTotal, 2)*15/100+grandTotal;
    ...
    calcGrandTotals: function () {
        var grandTotals = Number(jQuery($.opt.subtotal).html())
                       + Number(jQuery($.opt.shipping).val())
                       - Number(jQuery($.opt.discount).val());
                       grandTotals = self.roundNumber(grandTotals, 2)*15/100;

        jQuery($.opt.grandTotals).html(grandTotals);
     ...
     */
    newRow: function () {
        jQuery(".item-row:last").after('<tr class="item-row"><td><div class="delete-btn"><a class=' + $.opt.delete.substring(1) + ' href="javascript:;" title="Remove row"><i class="fa fa-times"></i></a></td><td class="item-name"><input  required autocomplete="off" name="pid[]" type="text" class="form-control barcode" placeholder="الباركود" type="text"></td><td class="item-name"><input  required autocomplete="off" type="text" name="product_name[]" class="form-control items" placeholder="الصنف" ></td><td class="item-name"><input  required autocomplete="off" type="text" class="form-control unit" name="unit[]" placeholder="الوحدة"></td><td><input  required autocomplete="off" class="form-control price" placeholder="الشراء" name="pays[]" type="text"> </td><td><input  required autocomplete="off" class="form-control price" placeholder="البيع" name="price[]" type="text"> </td><td><input class="form-control qty"  required autocomplete="off" name="qty[]" placeholder="الكمية" type="text"></td><td class="item-name"><input  required autocomplete="off" type="text" name="tax[]" value="15" class="form-control tax" placeholder="الضريبة" type="text"></div></td><td><span class="total">0.00</span></td></tr>');        if (jQuery($.opt.delete).length > 0) {
