...
if (isset($_POST["getNewOrderItem"])) {
	$obj = new DBOperation();
	$rows = $obj->getAllRecord("products limit 1");
	?>
	<tr>
...
?>
    ...
    <?php endwhile; ?>
		    ...
		    <td><input name="product_name[]" title="Product" type="text"   class="form-control pro_name" required></td>
		    <td><input name="unit[]" type="text"  title="Unit type"  class="form-control unit" required></td>
		    <td><input name="qty[]" title="الكمية الكلية للصنف الواحد" type="text" min="1"  class="form-control qty" required></td>
		    <td><input name="price[]" autocomplete="off"  title="سعر الشراء قبل الضريبة للوحده الواحده" type="text"   class="form-control price" ></td>
			<td><input name="pays[]" autocomplete="off" title="سعر البيع" type="text" required    class="form-control tell" >
			<input name="tax[]" value="<?php
...
$mx=mysql_fetch_array($nx); 
 echo $tax=$mx['tax'];
//نهاية الكود
?>" autocomplete="off" title="سعر البيع" type="hidden" required    class="form-control" ></td>
		   
</span>
<input name="pro_name[]" type="text" class="form-control form-control-sm pro_name">
			<input name="sum1[]" type="hidden"  id="sum1"  class="form-control sum1" required>
			<input name="sum2[]" type="hidden"  id="sum2"  class="form-control sum2" required>
			<input name="sum3[]" type="hidden"  id="sum3"  class="form-control sum3" required>
			<input name="date[]"  title="تاريخ التاريخ" type="hidden"  value="20<?php echo date('y-m-d');?>"  class="form-control" ></td>
	<span></tr>
<?php
	exit();
}
