		...
		foreach ($rows as $row) {
			?>
				<tr>
			        ...
			        <td><?php echo $row["product_tell"]; ?></td>
...
if (isset($_POST["update_product"])) {
	...
	$result = $m->update_record("products",["pid"=>$id],["cid"=>$cat,"bid"=>$brand,"product_name"=>$name,"unit"=>$unit,"product_price"=>$price,"tell"=>$tell,"product_stock"=>$qty,"added_date"=>$date]);
	echo $result;
}
...
if (isset($_POST["getNewOrderItem"])) {
	$obj = new DBOperation();
	$rows = $obj->getAllRecord("products limit 10");
	?>
	<tr>
...
?>
    ...
    <?php endwhile; ?>
		    ...
		    <td><input name="product_name[]" title="إسم الصنف" type="text"  style="border:0px; " class="form-control big-shadow pro_name" ></td>
		    <td><input name="unit[]" type="text" style="border:0px; " readonly="" class="form-control big-shadow unit" ></td>
		    <td><input name="qty[]" type="number" autocomplete="off" min="1" style="border:0px; " class="form-control big-shadow qty" ></td>
		    <td><input name="tell[]" type="number"  autocomplete="off"  min="1" style="border:0px; "  class="form-control big-shadow tell" >
		    </span>
		    <span><input name="pro_name[]" type="hidden" class="form-control big-shadow form-control-sm pro_name"></td>		   
		    <td><span class="amt">0</span> ريال </td>
		    <td><span class="amts">0</span> ريال </td>
		    <td><span class="brima">0</span> ريال </td>
		    <input name="sum1[]" type="hidden"  id="sum1" style="border:0px; " class="form-control big-shadow sum1" required><input name="sum2[]" type="hidden"  id="sum2" style="border:0px; " class="form-control sum2" required>
			<input name="sum3[]" type="hidden"  id="sum3" style="border:0px; " class="form-control big-shadow sum3" required>
			<input name="year[]" type="hidden"  id="sum3" style="border:0px; " value="20<?php echo date('y');?>" class="form-control" required></td>
	</tr>
	<?php
	exit();}
